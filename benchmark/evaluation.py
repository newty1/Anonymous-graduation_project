...
class MaskedDocument:
    ...
    doc_id: str
    masked_spans : List[Tuple[int, int]] # 元素是元组，元组中是被遮蔽文本的起始和结束位置

    def get_masked_offsets(self):
        """Returns the character offsets that are masked"""
        #返回被遮蔽的字符偏移量  masked_spans = [(5, 10), (15, 20)] {5, 6, 7, 8, 9, 15, 16, 17, 18, 19}
...
class TokenWeighting:
    ...
    def get_weights(self, text:str, text_spans:List[Tuple[int,int]]):
        ...
        text -- the text content in which the spans are located
        text_spans -- a list of (start, end) character boundaries for each span
        抽象类，计算每个文本span蕴含的信息权重大小
...
class AnnotatedEntity:
    """标注的文本中的同一个实体Represents an entity annotated in a document, with a unique identifier,
    ...
    entity_id: str
    mentions: List[Tuple[int, int]] #同一个实体在文本中出现范围的元组
    need_masking: bool
    is_direct: bool
    entity_type: str
    ...
    def __post_init__(self):
        """Checks that the entity is properly defined"""
    ...
    def mentions_to_mask(self):
        """根据标注返回应该被遮蔽的实体范围
        Returns the mentions that should be masked, according to the annotation"""
...
class GoldCorpus:
    ...
    表示Json格式TABTAB匿名化数据集"""

    def __init__(self, gold_standard_json_file:str, spacy_model = "en_core_web_lg"):
        """TAB匿名化数据集路径，spacy模型--中型模型"""
        # Loading the spacy model
        nlp = spacy.load(spacy_model)
        
        # documents indexed by identifier
        #创建golddocumen文件字典 键是doc_id
        self.documents = {}
        
        # Train/dev/test splits 存储测试集、验证集、训练集划分情况
        self.splits = {}
        ...
        for ann_doc in annotated_docs:
            ...
            #使用GoldDocument类存储文档 将Spacy文档传入
    ...
    def get_entity_recall(self, masked_docs:List[MaskedDocument], include_direct=True, 
                       include_quasi=True):
        """Returns the entity-level（实体级别的） recall of the masked spans when compared to the gold
        ...
        nb_masked_entities = 0 #被隐藏的实体数量
        nb_entities = 0   #全部实体数量
   ...
   #           %(include_direct, include_quasi)))
        ...
        for doc in masked_docs:
            ...
            gold_doc = self.documents[doc.doc_id]#golddocument
            
            entities_to_mask = gold_doc.get_entities_to_mask(include_direct, include_quasi)#返回所有需要被隐藏的实体（根据参数）
            masked_entities = [entity for entity in entities_to_mask if gold_doc.is_masked(doc, entity)]#返回golddoc（spacy）里面出现的隐藏后数量
    ...
    def get_recall(self, masked_docs:List[MaskedDocument], include_direct=True, 
        ...
        #返回了系统遮蔽的文本数量和总文本数量 #* 返回一个字典，键是实体类型，值是遮蔽的数量
    ...
    def _get_mask_counts(self, masked_docs:List[MaskedDocument], include_direct=True, 
        ...
        nb_elements_by_type = {}
        #初始化两个字典，一个存储遮蔽的数量，一个存储总数量，按照实体类型为键
...
class GoldDocument:
    ...
    def __init__(self, doc_id:str, text:str, annotations:Dict[str,List],
        ...
        """Creates a new annotated document with an identifier, a text content（text）, and
        a set of annotations (see guidelines)（annotator1orannotator2）"""
    ...
    def get_entities_to_mask(self,  include_direct=True, include_quasi=True):
        ...
        specified as arguments，且是指示符和准指示符"""
...
class UniformTokenWeighting(TokenWeighting):#继承了TokenWeighting，实现了权重为一的方法
...
class BertTokenWeighting(TokenWeighting):
    ...
    继承了TokenWeighting，实现了权重由bert计算的方法
    ...
    使用bert模型预测，如果一个标记越难被预测说明其信息内容越高权重越高
...
if __name__ == "__main__":
    ...
    gold_corpus = GoldCorpus(args.gold_standard_file)#加载gold标准数据集
    
    for masked_output_file in args.masked_output_file:
        print("=========")
        masked_docs = get_masked_docs_from_file(masked_output_file)#外部文件中的遮蔽文本

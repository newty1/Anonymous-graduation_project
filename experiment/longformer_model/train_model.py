...
if __name__ == "__main__":
    ...
    #使用之前的bert str 4096
    if device == 'cuda':
        criterion = nn.CrossEntropyLoss(ignore_index=-1, weight=torch.Tensor([1.0,10.0,10.0,10.0,10.0,10.0,10.0,10.0,10.0,10.0,10.0,10.0,10.0,10.0,10.0,10.0,10.0]).cuda())
    else:
        criterion = nn.CrossEntropyLoss(ignore_index=-1, weight=torch.Tensor([1.0,10.0,10.0,10.0,10.0,10.0,10.0,10.0,10.0,10.0,10.0,10.0,10.0,10.0,10.0,10.0,10.0]))
    ...
    try:
        while True:
            ...
            while True: # 找到开始位置，找到边界Whenever it finds an 1, it tries to find the boundary for this entity (stops at 0 or 1)
                ...
                if sv == 0 or sv%2 == 1:
                    break
            token_label = label_set.ids_to_label[l1[si-1]].replace("MASK", "")  # 将标签id转化为标签
    ...
    for i in out: # save the updated out {id: [(start,end)}
        if i[0] not in d:
            d[i[0]] = []
            d[i[0]].append((i[1],i[2],i[3]))#包括开始位置和结束位置
        else:

...
if __name__ == "__main__":
    ...
    try:
        while True:
            if sv==1:  #    遇到第一个B记录下fi 和sv            If an entity followed by another entity, fi, fv marks the beginning of an entity
                fi,fv = si,sv #fi记录实体的开始位置，fv记录实体的标签 作为实体的开始
            else:
                while True: #进入一个内层循环
                    fi,fv = next(it)
                    if fv:  #直到找到下一个实体的开始位置
                        break
            while True: # 找到开始位置，找到边界Whenever it finds an 1, it tries to find the boundary for this entity (stops at 0 or 1)
                si,sv = next(it)
                if sv == 0 or sv == 1:
                    break
            out.append((l2[fi][0],l2[fi][1],l2[si-1][2]))#fi[0]文件号，记录开始位置和结束位置

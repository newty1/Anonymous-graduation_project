...
def main():
    ...
    dataset = load_dataset("json", data_files={"train": train_file,
                                              "test": test_file})
    ##从命令行参数中获取训练文件和测试文件的路径。
    ##使用 Hugging Face 的 datasets 库加载训练和测试数据集。
import logging
import os
import sys
...
class ModelArguments:
    ...
    Arguments pertaining to which model/config/tokenizer we are going to fine-tune from.
     定义了一组与模型、配置和分词器相关的参数，用于指定要从哪个模型/配置/分词器进行微调。
    """
...
def check_words(prediction, target):
    ...
    :param prediction:
    :param target: 函数用于检查预测单词与目标单词之间的关系
...
def main():
    ...
    def compute_metrics(eval_preds):
        ...
        # Initialize counts for true positives (tps), false positives (fps), false negatives (fns), and true negatives (tns)
        fps, tps, fns, tns = 0, 0, 0, 0

        # Loop through each prediction-label pair
        for pred, label in zip(decoded_preds, decoded_labels):
            # Split the target and prediction into words
            target = label.split(" ")
            prediction = pred.split(" ")
            fp, tp, fn, tn = 0, 0, 0, 0
            for i in range(len(target)):
                # Check each word for true positives (tp), false positives (fp), false negatives (fn), and true negatives (tn)
                fp_i, tp_i, fn_i, tn_i = check_words(prediction[i], target[i])
                fp += fp_i
                tp += tp_i
                fn += fn_i
